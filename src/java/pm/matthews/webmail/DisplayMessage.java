...
public class DisplayMessage {
    ...
    public DisplayMessage(Store s, String f, int m, int st) {
        ...
        try {
            ...
            log.debug("can't GET message" + ex);
            doesExist = false;
        }
    ...
    public Multipart originalMessage() {
        ...
        try {
            ...
            if (ct.toLowerCase().startsWith("text/")) {
                ...
                p = new MimeBodyPart();
                p.setContent(message.getContent(), message.getContentType());
                mp.addBodyPart(p);
            }
            else {
                Object bd = message.getContent();
                mp = (Multipart) bd;
                if(removeAttachment){//we don't add a bodypart rather than trying to remove it 
                    Multipart temp = new MimeMultipart();
    ...
    public String[] messageDetails() {
        ...
        try {
            ...
            log.debug("error getting message details" + ex);
        }
        return msg;
    }
    ...
    private void displayHtml(Part p){
        try{
            ...
            log.debug("problem displaying HTML" + ex);
        }
    }
    ...
    private void displayRFC822(Part p){
        try{
            ...
            log.debug("problem displaying RFC8222" + ex);
        }
    }
    ...
    private void displayInlineImage(Part p){
        try{
            StringBuilder image = new StringBuilder("<img src='data:");
            String temp = p.getContentType().toString();//this may include a filename which we do not want here
            ...
            image.append(data);
            image.append("'/><br>");
            body.append(image.toString());
        }
        catch(Exception ex){
            log.debug("problems displaying inline IMAGE" + ex);
        }
    }
    ...
    private void displayDownloadLink(Part p){
        try{
            ...
            log.debug("problems with download LINK for " + ex);
        }
    }
