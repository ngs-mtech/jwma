...
public class FolderAction extends BaseAction implements ValidationErrorHandler{
    ...
    public Resolution logout(){
        ...
        return new ForwardResolution("/WEB-INF/desktop/logout.jsp");
    }
    ...
    public Resolution listMessages(){
        ...
        updateMailSession();
        String view = "/WEB-INF/mobile/messagelist.jsp";
        if(!jession.mobile){
            view = "/WEB-INF/desktop/messagelist.jsp";
        } 
    ...
    public Resolution deleteFolder(){
        ...
        for(String s:selection){
            try{
                ...
                if(!f.getName().equals(jession.draft.toString())&&!f.getName().equals(jession.sent.toString())&&!f.getName().equals(jession.trash.toString())){
                    f.delete(true);
                    log.info("folder " + s + " DELETED");
                }
                else{
                    LocalizableError err = new LocalizableError("storename.isSystem");
                    getContext().getMessages().add(err);
                    log.error(s + " is a SYSTEM folder");
                }
            }
    ...
    public Resolution moveFolder(){
        ...
        try{
          for(String s:selection){
            ...
            log.debug("CAN'T move folder "+ ex);
        }           
        jession.getMixedStoreList();
        return new RedirectResolution(BaseAction.class);
    }
    ...
    public Resolution searchWhat(){
        ...
        sent = folder.equals(jession.sent);
        prepareMessageList(true, sent);       
        return new ForwardResolution("/WEB-INF/desktop/messagelist.jsp");
    }
    ...
    public String getKeepAlive(){return (String)getSessionValue("keep_alive");}
