...
public class SendAction  extends BaseAction{
    ...
    public Resolution saveDraft(){
        jession = getSessionValue("jession");
        String view = "/WEB-INF/desktop/folder.jsp";
        //FIXME:see mobile/sendmail.jsp
        if(jession.mobile){           
            sendMail();
            view = "/WEB-INF/mobile/folder.jsp";
        }
        else{
            updateMailSession();
    ...
    private boolean writeMessage(){
        ...
        if(msgStatus){
            if(attachment == null && forward == null){
                ...
                sm.writeAttachment(body, attachment, home, richText);
            }
            else{
    ...
    private void messageFrom(){
        ...
        if(autoSign){
            ...
            body =  body + line + from.replace(ob, cr) + cr + cr;
        }
