...
public final class JwmaSession {
    ...
    public int messages, unread, trashMessages, sortOrder, processor;
    ...
    JwmaSession(Session ms, Store st, Preference p){
        ...
        catch(Exception ex){log.debug("kwaaaaaa cannot find INBOX");}
        updateInbox();   
        createMailbox(sent);
        createMailbox(draft);
        getMixedStoreList();
    ...
    public void updateTrash(){
        try{
            ...
            trashMessages = trash.getMessageCount();
            trashEmpty = trashMessages == 0;
            trash.close(false);
        }
        catch(Exception mex){
            log.debug(mex);
