...
public class SendMessage {
    ...
    public void writeAttachment(String body, List<FileBean> list, String home, boolean richText){
        try{
            ...
            for(FileBean fb:list){
                if(fb != null){
                    ...
                    String filePath = home+File.separator+"uploads"+File.separator+fileName;
                    fb.save(new File(filePath));
                    DataSource fds = new FileDataSource(filePath);
                    MimeBodyPart attachment = new MimeBodyPart();  
                    attachment.setDataHandler(new DataHandler(fds));  
                    attachment.setFileName(fileName);
                    multipart.addBodyPart(attachment, multipart.getCount());
    ...
    private Address[] getRecipients(String s){
        ...
        //(1)preamble before comma (2)comma (3)preable after comma 
        //(4)none or more whitespace chars (5)actual email address
        Matcher m = p.matcher(s);
        while(m.find()){
            s = m.group(1) + "|" + m.group(3) + m.group(4) + m.group(5);
        }
