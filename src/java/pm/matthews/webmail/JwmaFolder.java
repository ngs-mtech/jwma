...
public class JwmaFolder {
    ...
    public void emptyTrash(){
        try{
            ...
            log.debug("failed to EMPTY trash " + ex);
        }
    }
    ...
    public void deleteMessage(List<String> delete){
        try{
            ...
            log.debug("FAIL in deleteMessage() "+ex);
        }
    }
    ...
    public void moveMessage(List<String> move, String destination){
        ...
        try{
            ...
            folder.close(true);  
            sortMovedMessages(target);  
        }
        catch(Exception ex){
            log.debug("FAIL in moveMessage() "+ex);
        }
    }
    
    /* some very weird code, but I can't find another way to do this
     * 
     * java version "1.7.0_111"
     * OpenJDK Runtime Environment (IcedTea 2.6.7) (7u111-2.6.7-1~deb8u1)
     * OpenJDK 64-Bit Server VM (build 24.111-b01, mixed mode)
     * 
     * folder.appendMessages() does not work for me on either a closed or open folder:-
     * FAIL in sortMovedMessages() javax.mail.FolderClosedException
     * java.lang.IllegalStateException: This operation is not allowed on an open folder
     * 
     * copying a sorted array in one go does not preserve the order!!
     */
    private void sortMovedMessages(Folder folder){
        try{
            folder.open(Folder.READ_WRITE);
            ...
            for(i = 0; i < count;i++){
                msg[i] = folder.getMessage(i +1);
            }

           //sort array by date
           Date d1, d2;
           Message temp;
           Message[] copy = new Message[1];
           for(i = 0; i < count-1;i++){
                for(j = i+1; j < count; j++){
                    d1 = msg[i].getReceivedDate();
                    d2 = msg[j].getReceivedDate();
                    if(d1.after(d2)){
                       //log.info("swapped" + d1 + " for "+d2);
                       temp = msg[i];
                       msg[i] = msg[j];
                       msg[j] = temp;                  
                   } 
               }
               //copying messages one by one back to the end of the folder
               //copying the sorted array in one go *does not* preserve its order!!
               copy[0] = msg[i];
               folder.copyMessages(copy, folder);
            }
            //copy the last message from the sorted array
            copy[0] = msg[i];
            folder.copyMessages(copy, folder);
            
            //now delete the original unsorted messages
            for(i = 0; i < count;i++){
                msg[i] = folder.getMessage(i +1);
            ...
            log.debug("FAIL in sortMovedMessages() "+ex);
        }
    }
